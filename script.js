document.addEventListener('DOMContentLoaded', function () {
  ...
  document.body.addEventListener('submit', function (e) {
    ...
    fields.forEach(function (field) {
      ...
      if (field.hasAttribute('required') && value === '') {
        ...
        errorMessage = getFieldLabel(field, form) + ' must be at least ' + field.minLength + ' characters.';
      } else if (field.maxLength && field.maxLength > 0 && value.length > field.maxLength) {
        errorMessage = getFieldLabel(field, form) + ' must not exceed ' + field.maxLength + ' characters.';
      }

      if (field.tagName === 'SELECT' && field.hasAttribute('required') && (value === '' || value === null)) {
        errorMessage = 'Please select an option for ' + getFieldLabel(field, form) + '.';
      }
  ...
  function clearFormErrors(form) {
    ...
    const invalidFields = form.querySelectorAll('.invalid, .valid');
  ...
  function initScrollAnimations() {
    ...
    if ('IntersectionObserver' in window) {
      ...
      elements.forEach(function (el) {
        el.classList.add(OBSERVE_CLASS, ANIMATION_CLASS);
      });
    }
  }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "Top-Binian GROUP משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: 'aaa43192-8623-4d46-976e-e1072c1087be',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || 'aaa43192-8623-4d46-976e-e1072c1087be';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'aaa43192-8623-4d46-976e-e1072c1087be';
